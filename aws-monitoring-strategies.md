#### All services in AWS have their own documentation portal at **``https://aws.amazon/com/<servicename>``**
  + Example for CloudWatch:
    + https://aws.amazon.com/cloudwatch/
  + All other services mentioned have similar URLs

#### Different ways of managing access control to S3:

  + https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html
  + https://docs.aws.amazon.com/AmazonS3/latest/dev/using-iam-policies.html
  + https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html
  + https://aws.amazon.com/about-aws/whats-new/2018/11/introducing-amazon-s3-block-public-access/

#### CloudWatch support in AWS
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CW_Support_For_AWS.html

#### CloudWatch Dashboards
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch_Dashboards.html
  + https://aws.amazon.com/about-aws/whats-new/2018/11/amazon-cloudwatch-introduces-automatic-dashboards-to-monitor-all-aws-resources/

#### CloudWatch Agent install
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Install-CloudWatch-Agent.html

#### Legacy Perl CloudWatch agent
  + https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/mon-scripts.html

#### CloudWatch Agent automated install
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/install-CloudWatch-Agent-on-first-instance.html
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/install-CloudWatch-Agent-on-first-instance.html#CW-Agent-Instance-Create-Configuration-File-first
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/create-cloudwatch-agent-configuration-file-wizard.html

#### Route 53 Failover
  + https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/dns-failover-configuring.html

#### CloudWatch Logs
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/WhatIsCloudWatchLogs.html
  + https://aws.amazon.com/blogs/aws/new-amazon-cloudwatch-logs-insights-fast-interactive-log-analytics/

#### CloudWatch Agent summary
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/AgentReference.html

#### CloudWatch Alarm details
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/AlarmThatSendsEmail.html

#### CloudWatch metric filters
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/FilterAndPatternSyntax.html

#### CloudWatch Events summary
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/events/WhatIsCloudWatchEvents.html

#### CloudWatch Event types
  + https://docs.aws.amazon.com/AmazonCloudWatch/latest/events/EventTypes.html

#### CloudTrail unsupported services
  + https://docs.aws.amazon.com/awscloudtrail/latest/userguide/cloudtrail-unsupported-aws-services.html

#### AWS Config summary
  + https://docs.aws.amazon.com/config/latest/developerguide/resource-config-reference.html

#### AWS Config Rules
  + https://docs.aws.amazon.com/config/latest/developerguide/managed-rules-by-aws-config.html

#### VPC Flow Logs summary
  + https://docs.aws.amazon.com/vpc/latest/userguide/flow-logs.html

#### SSM Features
  + https://aws.amazon.com/systems-manager/features/
